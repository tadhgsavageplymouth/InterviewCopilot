...
def main():
    ...
    # 3) Accept exactly one client at a time, but keep talking until they close
    while True:
        conn, addr = server.accept()
        print 'Client connected from', addr

        try:
            # 4) Inner loop: handle *multiple* messages in the same connection
            while True:
                try:
                    data = conn.recv(1024)
                ...
                # recv() returns empty string only when the client closed
                if not data:
